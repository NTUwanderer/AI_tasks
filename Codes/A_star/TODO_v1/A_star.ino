...
int A_star_search(State state, int cutoff, vector<Step>& steps) {
    ...
    /*
        ...
        queue.top: return the top element (in this case, state with smallest f)
    ...
    */
    ...
    while (!(queue.empty())) {
        ...
        if (s == goalState) {
            // TODO: Store the optimal path in steps
        ...
        for (int i = 0; i < avSteps.size(); ++i) {
            ...
            // TODO: Check if the successor should be added to the queue or update the queue
            KeyType key = getKey(successor);

            if (frontier.find(key) != frontier.end()) {
                // TODO
            } else {
                ++count; // New state, increment the count to record
                // TODO
            }
        }
    }
