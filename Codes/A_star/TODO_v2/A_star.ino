...
void setup() {
    ...
    // TODO: after testing BFS/GS/A*, use millis() instead of 1 as the random seed
    // randomSeed(millis());
    randomSeed(1);
...
int A_star_search(State state, int cutoff, vector<Step>& steps) {
    ...
    FMap frontier; // key to heuristic f (BFS: g, GS: h, A*: f) 

    // TODO: Initial setup of above queue or hashtables...?
    /*
        Functions you may use:

        getKey(state): get key of the state based on permutation of 1 ~ 8 only
        getHash(state): get hash of the state based on permutation & f (compression of state)
        getState(hash): decompress the state from the hash
        takeStep(state, step): return the new state as the given state takes the give step
        unordered_map.map[key] = value: store the key & value pair in the hashtable
        unordered_map.erase(key): erase the stored key & value pair in the hashtable
        unordered_map.find(key): return true if the key exists in the hashtable, else false
        queue.push(state): add the state into the queue
        queue.top(): return the top (largest) element (but we want the state with smallest f)
        queue.pop(): erase the top element from the queue
    */

    while (!(queue.empty())) {
        yield();
        // TODO: Pop state from queue & update explored and frontier
        ...
        for (int i = 0; i < avSteps.size(); ++i) {
            ...
            // TODO: Check if the successor should be added to the frontier or update the frontier
            ...
            if (frontier.find(key) != frontier.end()) {
                 ...
                 * If so, remove successory in queue & re add it, update frontier and prevStep
                 * queue.remove(state): remove the state identified by pos
                 */
            } else {
                /* TODO
